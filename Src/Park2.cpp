...
//costruttori
Park2::Park2(int cap):capienza{cap},prossima_entrata{1900, /*Date::Month::jan*/1,1 ,0 ,0},prossima_uscita{1900, /*Date::Month::jan*/1,1 ,0 ,0}, auto_presenti(/*cap*/1), numero_auto{0}, pos{auto_presenti.begin()}/*, finite_auto_che_escono{false}, finite_auto_che_entrano{false}*/,uscite_sono_finite{false}, entrate_sono_finite{false}{}
Park2::Park2():capienza{0},prossima_entrata{1900, /*Date::Month::jan*/1,1 ,0 ,0},prossima_uscita{1900, /*Date::Month::jan*/1,1 ,0 ,0}, auto_presenti(0), numero_auto{0},pos{auto_presenti.begin()}/*, finite_auto_che_escono{false}/*, finite_auto_che_entrano{false}*/,entrate_sono_finite{false},uscite_sono_finite{false}{}
//funzioni
...
void Park2::fai_entrare_auto(Auto& car){
    unique_lock<std::mutex> mlock(mutex_);
    ...
    if(numero_auto == capienza){
        ...
        //cout << car.get_data_ingresso() << ' ' << car.get_targa() << ' ' << "IN " << "PARK2" << endl; //prima c'era *pos
        //pos++;
    }
}
...
void Park2::fai_uscire_auto(Auto car){
    unique_lock<std::mutex> mlock(mutex_);
...
void Park2::set_uscite_finite(){
    unique_lock<std::mutex> mlock(mutex_);
    uscite_sono_finite = true;
    mio_turno_per_entrare.notify_one();
}

void Park2::set_entrate_finite(){
    unique_lock<std::mutex> mlock(mutex_);
    entrate_sono_finite = true;
    mio_turno_per_uscire.notify_one();
}

void Park2::print_lista_auto(){
    list<Auto>::iterator index = auto_presenti.begin();
    cout << "Lista auto presenti nel PARCHEGGIO 2 alla fine dell'esecuzione:" << endl;
